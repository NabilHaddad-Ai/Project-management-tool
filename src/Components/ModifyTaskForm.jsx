...
export default function ModifyTaskForm({ onSubmit, onCancel , LastTitle , LastSubtitle , lastPriority}) {

  const [title, setTitle] = useState(LastTitle);
  const [subtitle, setSubtitle] = useState(LastSubtitle);
  const [priority, setPriority] = useState(lastPriority); // "Low" | "Medium" | "High"
  
  
  function handleSubmit(e) {
    e.preventDefault();
    if (!title.trim()) return;
    //we pass the title , subtitle and priority to the parent component
